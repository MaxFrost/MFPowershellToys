function Install-RemoteMsi {
    Param(
        # Name of target computers to deploy software to
        [CmdletBinding()]
        [Parameter(
            Mandatory=$true,
            Position=0,
        ...
        # Path to Installer file to be deployed to remote machines. Requires that installer be capable of silent installation
        [Parameter(
            Mandatory=$true,
            Position=1
            )
        ...
        # Commandline Arguments for installer
    ...
    process {
        ...
        Write-Verbose "Finished Deploying $InstallFile to all targets"
    }
}